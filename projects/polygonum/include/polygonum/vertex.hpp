...
protected:
	...
	virtual void getRawData(VertexSet& destVertices, std::vector<uint16_t>& destIndices, vec2<std::shared_ptr<Texture>>& textures) = 0;   //!< Get vertexes and indices from source. Subclasses define this.
...
public:
	...
	void loadVertexes(VertexData& result, vec2<std::shared_ptr<Texture>>& textures, Renderer& r);   //!< Get vertexes from source and store them in "result" ("resources" is used to store additional resources, if they exist).
};
...
class VL_fromBuffer : public VertexesLoader
{
	...
	void getRawData(VertexSet& destVertices, std::vector<uint16_t>& destIndices, vec2<std::shared_ptr<Texture>>& textures) override;
...
class VL_fromFile : public VertexesLoader
{
	...
	vec2<std::shared_ptr<Texture>>* textures;   //!< Stores textures in case they're included by the file
